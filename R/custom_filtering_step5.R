...
calculate_cons_scoring_data <- function(alignment_id, cons_clade, feat_length,
    ...
    # TODO: ver que esto asigna un id, no puedo hacerlo separado por cromosoma porque se van a repetir los ids
    # conserved elements filtered:
    cons_elem_path <- calculate_conserved_elements(alignment_id, cons_clade,
                                                   chr)

    conservation_data <- load_conserved_elements(file_path = cons_elem_path,
                                                 format = 'data.frame')
    conservation_gr <- GenomicRanges::makeGRangesFromDataFrame(
        conservation_data, keep.extra.columns = TRUE)

    acc_scoring_data <- load_acc_filtered_scoring(file_path = acc_scoring_path)
    acc_scoring_gr <- GenomicRanges::makeGRangesFromDataFrame(acc_scoring_data,
    ...
    shift_count_in_cons_elem <- combined_data |>
        dplyr::group_by(cons_id) |>
        dplyr::summarise(shift_count_in_cons_elem = sum(acc_acc_element_shift_count))

    tmp <- dplyr::inner_join(acc_elements_count_in_cons_elem, shift_count_in_cons_elem,
                             by = "cons_id")

    gap_count_in_cons_elem <- combined_data |>
        dplyr::group_by(cons_id) |>
        dplyr::summarise(gap_count_in_cons_elem = sum(acc_acc_element_gap_count))
    ...
    acceleration_conservation_result <- dplyr::inner_join(conservation_result,
                                                          combined_data[, combined_data_to_save_cols],
                                                          by = c('phastCons_id' = 'cons_id',
                                                                 'acc_id' = 'acc_id'))


    out_file_path <- save_candidate_elements_raw_scoring(acceleration_conservation_result,
                                                         alignment_id, cons_clade,
                                                         feat_length, chr)
